...
export default function LoginWindowApp() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      window.electronAPI?.send('login-confirm', { type: 'user-login-confirmed', email: initialEmail });
    }, 5000);
  ...
  const handleSubmit = async (values: LoginFormValues) => {
    ...
    try {
      ...
      window.electronAPI.send('login-confirm', { type: 'user-login-confirmed', email });
    } catch (err) {
      setLoading(false);
