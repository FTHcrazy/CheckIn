...
- **地图河道「不连贯 / 水里长山」**（`MapPage`，用户反馈三连之一）：根因是**同一片水被画了两遍** ——
  GL 拿 `cells` 里的河格刷出一片宽水面（域扰动 + 覆盖度模糊把 1 格河吹到 ≈3 格），
  2D 层再按 `pts` 叠一条 0.55/0.34 格的双层水带，两者中心线与宽度都对不上；
  且**支流只存了 `pts`、没刻进 `cells`**，于是支流成了「悬在陆地上的水带」。
  修复：
  ① `addTributaries` 把支流刻进 `cells`（交汇点与主干共格，真正汇流）；
  ② 新增 `fillRiverBase()` 产出 `BuiltWorld.baseCells`（河格回填为最近陆地地形），
  GL splat 与 Canvas2D 底色/等值线一律改读它 ⇒ **河格不再产生水面**；
  ③ `drawRiverRibbons` 重写为**水体唯一几何**：单条闭合变宽轮廓（源头 0.15 格 → 河口 0.30 格、
  支流反向收细），曲线平滑 + 正弦横向摆动打散栅格楼梯，岸线恒定 1.1px 屏幕宽，
  远景守卫（最粗处不足 1.5px 整条跳过）。标题症状「两条水并排、断头胶囊」消失。
- **地形图标长在水里**：`drawTerrainIcons` 只看 `cells` 地形码是不够的 —— GL 的实际画面由
  **模糊后的软场**决定。新增 `buildGlWaterMask()`（软场格中心读数 + 加权最大，与 shader 同规则），
  图标层据此跳过「GL 渲染为水面」的格；掩码在 MapCanvas 里按 world 缓存。
- **缩放时地图尺寸标注不变化**（比例尺）：旧实现是写死的 150px 刻度条 + 固定文案。新增纯函数模块
  `map-scale.ts`（`scaleBarFor` / `viewExtentLi` / `niceFloor` / `formatLi`）：刻度条屏幕长度保持
  ~150px，**里程取「1/2/5 × 10ⁿ 里」随缩放逐档变化**；状态条新增实时「视野 W×H 里」，
  `1 格 = 25 里` 收敛到 `LI_PER_CELL` 单一常量。
- 图表定点：`map-terrain.test.ts` 新增支流刻格 / `fillRiverBase` 两组（10 项）；
  `map-render.test.ts` 新增「一条河 = 一次 fill + 一次 stroke」「河宽沿程单调」「远景守卫」
  「地表不出现河流蓝」「掩码命中不落笔」（6 项）；新增 `map-scale.test.ts`（10 项）。
  修复前后对照图：`designs/novel-map/river-fix-before-after.png`。

### Changed

- **地图地形图标：稀疏化 + 去栅格**（`MapPage`，向"游戏级手绘地图"观感推进的第一步）：
  GL 层的逐地貌程序化材质（林冠团块 / 岩脊 / 沙丘条纹 / 水洼 / 岩浆龟裂）早已承担质感，
  而 2D 补画层仍是**一格一个、钉死格心、等大朝上**地铺图标 —— 等于把 GL 用域扰动
  好不容易揉掉的栅格感原样压回去，这是与设计稿观感差距最大的一处。改为**抖动栅格抽样**
  （`map-render.ICON_SAMPLE`，step=2 / jitter=±0.55 格 / 边长 1.05~1.55 格 / 旋转 ±0.1rad），
  位置·尺寸·旋转各由独立哈希派生，与迭代顺序无关且同 seed 必复现。
  ⚠️ **落笔前按抖动后的中心重取所在格**再判水体：抖动会跨格，沿用候选格判定会把树送进海里。
  实测 64×64 纯色世界候选点 1023/4096（25.0%），叠加纹理原有概率门后
  森林 ≈15%（旧 62%）、山地 ≈18.5%（旧 74%）、沙漠 25%（旧 100%）、雪原 ≈5.5%（旧 22%）；
  绘制调用数随之降到 1/4。`drawTerrain`（无 GL / 地块模式回退）保持逐格满密度不动 ——
  那条路径上图标**就是**地表材质本身，抽稀会让回退图退化成纯色块。
  新增 `sampleIconSites()` 纯函数（便于单测），`map-render.test.ts` 加 8 项守卫
  （稀疏度 ≤1/4、水体零候选、边长≠一格且不等大、中心被抖离候选格、旋转双向、抖动跨格后不入水、确定性）。
...
- **地图格局模板扩到 16 套（新增 9 套热门小说格局）**：`map-config.ts` 的
  `TEMPLATE_OPTIONS` 按 `base` / `hot` 两组渲染 —— 基础组沿用原有的中央大陆、环海群岛等；
  新增热门小说组：**十万大山**（山地占比 ≥ 28.5%，山脊成片）、**南疆沼泽**（湿地偏南集中，
  marsh 占比 ≈ 7.3%）、**赤炎火山**（lava ≈ 6.7%）、**裂谷天堑**（中线整条低于全图）、
  **群岛海域**（sea > 58%）、**东荒戈壁**、**古战场废墟**（ruins ≈ 9.6%）、**千里平原**
  （山地 ≥ 0%，只留孤立缓丘）、**中央大陆（九州）**。
- **地形类型扩到 12 类**：新增 `T_MARSH`（沼泽）、`T_RUINS`（废墟），恰好占满
  `3 × RGBA = 12` 个 splat 通道，无需扩贴图张数；`TER_KEY` / `TER_META` /
  `TERRAIN_COLORS` / `TERRAIN_BORDER_COLORS` / `TERRAIN_TEXTURE` 同步补齐
  （marsh `#5c7d63`、ruins `#b3a795`，纹理新增 `wetland` / `rubble`）。
- **地图图貌增强**：GPU 侧新增**地形晕渲（hillshade）** —— 用「非水面占比」的 x/y 双向差分
  当坡向，光照明暗制造起伏感；色板增加纸面底色 `MAP_PAPER` 与图框色 `MAP_FRAME`；
  新增 `drawMapFrame()` 双线画框 + 四角折角装饰；12 种地块纹理全部重绘（树加树冠/树干/
  高光，山加受光面/山脚阴影，雪山加冷色侧影）；地点符号新增宗门 / 秘境 / 魔渊，
  装饰新增火山 / 农田 / 断垣（`arch`）。
- **叠加型符号改为派生式生成**：新增纯函数 `buildOverlayFeatures()`，岛屿 / 瀑布
  全部由 `cells` 确定性派生，不再写入存档，存档只保留河流（含 `pts`）。
  `worldFromContent()` 重写为「只读回 river features + 重派生叠加层」。
  （瀑布的跌落方向在派生时按低侧法线算好，存进 `TerrainFeature.angle`。）
- **图例走单一事实源**：新增 `terrainLabel()` / `terrainDesc()`（读 `TER_META`），
  图例、侧栏、导出图条统一调用；`LEGEND_VERSION` 升到 2。

### Removed

- **悬崖地形 / 符号整体移除**（叠加型由 3 类降为 2 类，地形总数 15 → 14）：
  高差格边上覆一层折线 + 崖齿，实测在成片山地 / 雪原边缘会沿等高线糊出一圈圈
  「梳齿状」黑线（用户截图红框处），既不像地貌又把地形本身压住了。
  已删除 `drawCliff()`、`OVERLAY_TYPES` 里的 `cliff`、`TerrainFeature` 类型联合成员，
  以及配套的整套格边串链逻辑（邻接表 walk / `canonicalize` / `reversePolyline`）；
  山地起伏改由 GL 侧的地形晕渲（hillshade）表达。旧档里的 `cliff` 字段读取时被忽略，
  打开即自愈；瀑布跌落方向随之改为派生时按低侧法线算好，不再依赖崖线。

### Fixed

- **GL 模式下地形「没有贴图」、认不出地貌**：GL 路径的上层 Canvas2D 只画了河流 ribbon /
  叠加符号 / 图框，**逐格地形纹理一个都没画** —— 整张图退化成一片渐变色块，
  森林与草原、沙漠与废墟只能靠颜色猜（GL 侧程序化纹理振幅仅 ±5%~18%，在
  80×50 / 千像素级下早已糊成噪点）。新增 `drawTerrainIcons()` 在 GL 层之上补画陆地
  地形图标（树 / 沙丘 / 山脊 / 雪帽 / 冰晶 / 岩浆 / 水洼芦苇 / 碎石断墙），水体跳过
  （GL 已有动态水纹）；并加「每格屏幕像素 < 4px 不画」的细度守卫，避免远景白耗绘制预算。
- **悬在水面中央的奇怪双竖线（瀑布误判）**：旧判定只要求「河点邻域有山地」，
  河道远离高差处仍会生成，渲染成悬空图案。改为用 `forWater` + `lipX` 精确判定
  「河流中心格紧邻高差格边」，落点吸附格边中点，间隔 ≥ 8 格、全图至多 2 处。
- **旧存档重开后整条河流消失**：旧实现漏掉 `pts` 透传，`worldFromContent` 读回后
  河流 ribbon 无法绘制。现在 `parseContent` 归一化旧档（丢弃非 river features），
  `worldFromContent` 完整读回 `pts` / `trib`。
